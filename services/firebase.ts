...
import { Character, Project, AppSettings, Location } from "../types";
...
export const saveProjectToFirestore = async (userId: string, project: Project) => {
  ...
  if (cleanProject.panels && Array.isArray(cleanProject.panels)) {
    cleanProject.panels = cleanProject.panels.map((p: any) => ({
      ...p,
  ...
  console.log(`[Firebase] Saving project ${project.id} for user ${userId}. Panels: ${cleanProject.panels?.length}`);
  try {
...
export const uploadPanelImageFromString = async (userId: string, dataUrl: string): Promise<string> => {
  console.log(`[Firebase] Uploading image string for user ${userId}...`);
  const storageRef = ref(storage, `users/${userId}/panels/${Date.now()}.png`);
...
export const uploadPanelVideoFromString = async (userId: string, dataUrl: string): Promise<string> => {
  console.log(`[Firebase] Uploading video string for user ${userId}...`);
  const storageRef = ref(storage, `users/${userId}/panels/${Date.now()}.mp4`);
...
export const uploadPanelImageFromFile = async (userId: string, file: File): Promise<string> => {
  const storageRef = ref(storage, `users/${userId}/panels/${Date.now()}_${file.name}`);
...
export const uploadPanelAudio = async (userId: string, base64Data: string): Promise<string> => {
  const storageRef = ref(storage, `users/${userId}/audio/${Date.now()}.wav`);
